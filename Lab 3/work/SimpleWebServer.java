...
public class SimpleWebServer {                            
    ...
    public void storeFile(BufferedReader br,
		...
		try {
		    ...
		    while (s != null) {
		    	if (!isMeta(s.trim()) && !isEmpty(s.trim())) {
		    		fw.write (s);
		    	}
			...
			osw.flush(); // ensure response is fully sent
		}
		catch (Exception e) {
    ...
    public boolean isMeta(final String s) {
    	return (s.startsWith(HOST) || s.startsWith(USER_AGENT) || s.startsWith(ACCEPT) || 
    		s.startsWith(CONTENT_TYPE) || s.startsWith(CONTENT_LENGTH) );
    }
